package com.example.memorypractice.app.repeat.service;

import com.example.memorypractice.app.repeat.RepeatCompleteRepository;
import com.example.memorypractice.app.repeat.RepeatTodoEntity;
import com.example.memorypractice.app.repeat.RepeatTodoRepository;
import com.example.memorypractice.app.repeat.RepeatType;
import com.example.memorypractice.app.repeat.resdto.ResAlltodo;
import com.example.memorypractice.app.repeat.resdto.ResCompleteDay;
import com.example.memorypractice.app.repeat.resdto.ResRepeatList;
import com.example.memorypractice.app.repeat.resdto.ResRepeatMonthlyCount;
...
public class RepeatR_Service {
    ...
    public List<ResAlltodo> allTodo(Long userId, LocalDate date) {

        // 투두목록 불러오기
        List<TodoEntity> todoEntities = todoRepository.findByUser_IdAndDueDate(userId,date);

        // 리핏목록 불러오기
        List<RepeatTodoEntity> repeatTodoEntities = repeatRepository.findByUserIdAndDate(userId,date,date.getDayOfWeek(),RepeatType.DAILY,RepeatType.WEEKLY);

        Set<Long> completedRepeatIds = completeRepository.findCompletedRepeatTodoIds(userId, date);
        List<ResAlltodo> result = new ArrayList<>();
        for (TodoEntity todo : todoEntities) {
            result.add(new ResAlltodo(
                    todo.getId(), null, todo.getTitle(), todo.getMemo(), todo.isCompleted(),
                    todo.getPriority(), todo.getDueDate(), null, Set.of()
            ));
        }
        for (RepeatTodoEntity repeatTodo : repeatTodoEntities) {
            result.add(new ResAlltodo(
                    null, repeatTodo.getId(), repeatTodo.getTitle(), repeatTodo.getMemo(),
                    completedRepeatIds.contains(repeatTodo.getId()), null, date,
                    repeatTodo.getRepeatType(), repeatTodo.getDayOfWeek()
            ));
        }
        return result;
