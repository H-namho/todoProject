package com.example.memorypractice.shared.redis;
...
public class TokenRedisRepositry {
    ...
    public String invalidBeforeKey(Long userId) {
        return "invalidBefore:" + userId;
    }
    ...
    public void invalidateTokensIssuedBefore(Long userId, long timestamp) {
        redisTemplate.opsForValue().set(invalidBeforeKey(userId), String.valueOf(timestamp));
    }

    public boolean isTokenInvalidated(Long userId, long issuedAt) {
        String invalidBefore = redisTemplate.opsForValue().get(invalidBeforeKey(userId));
        return invalidBefore != null && issuedAt <= Long.parseLong(invalidBefore);
    }
}
